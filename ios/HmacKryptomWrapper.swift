...
public struct HmacKryptomWrapper {
    ...
    static func sign(algorithmIdentifier: String, key: Data, data: Data) async throws -> Data {
        let importedKey = try HmacKey(rawKey: NSDataUtilsKt.toByteArray(key), algorithm: HmacAlgorithmCompanion.shared.fromIdentifier(identifier: algorithmIdentifier))
